...
// add your server's IP and open your firewall
    if(inet_pton(AF_INET, "47.94.157.136", &server_addr.sin_addr) == 0)
    {
...
*/
    ...
    bzero(buffer, BUFFER_SIZE);
    int length = 0;
    while((length = recv(client_socket_fd, buffer, BUFFER_SIZE, 0)) > 0) //correspond to send(3)
    {
	//printf("####L(%d)length=%d\n", __LINE__, length);//server
